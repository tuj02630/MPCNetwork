...
class Notification(Data):
    ...
    COLUMNS = [TYPE, CRITERIA_TYPE, ID]
    EXPLICIT_TYPE = f"{TABLE}.{TYPE}"
    EXPLICIT_ID = f"{TABLE}.{ID}"
    EXPLICIT_CRITERIA_TYPE = f"{TABLE}.{CRITERIA_TYPE}"
    EXPLICIT_COLUMNS = [EXPLICIT_TYPE, EXPLICIT_ID, EXPLICIT_CRITERIA_TYPE]

    def __init__(self, notification_type: int, criteria_type: int, notification_id: int = None):
    ...
    def __str__(self):
        return "[Notification   ]               :NOTIFICATION_TYPE: {:<12} CRITERIA_TYPE: {:<12} " \
               "NOTIFICATION_ID: {:<12}" \
            .format(self.notification_type, self.criteria_type, self.notification_id)

    @staticmethod
    def dict_to_object(payload: dict, explicit=False) -> "Notification":
        if explicit:
            return Notification(payload[Notification.EXPLICIT_TYPE],
                                payload[Notification.EXPLICIT_CRITERIA_TYPE], payload[Notification.EXPLICIT_ID])
        else:
            return Notification(payload[Notification.TYPE],
                                payload[Notification.CRITERIA_TYPE], payload[Notification.ID])
